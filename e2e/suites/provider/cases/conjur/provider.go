...
func (s *conjurProvider) BeforeEach() {
	...
	// s.CreateJWTStore(c, ns)
	// s.CreateJWTK8sStore(c, ns)
	// s.CreateKubernetesAuthStore(c, ns)
}
...
// func makeClusterStore(name, ns string, c *addon.Conjur) *esv1beta1.ClusterSecretStore {
// 	store := makeStore(name, ns, c)
...
// func (s conjurProvider) CreateJWTStore(c *addon.Conjur, ns string) {
// 	conjurCreds := &v1.Secret{
// 		ObjectMeta: metav1.ObjectMeta{
...
// 	err := s.framework.CRClient.Create(context.Background(), conjurCreds)
// 	Expect(err).ToNot(HaveOccurred())
// 	secretStore := makeStore(jwtProviderName, ns, c)
// 	secretStore.Spec.Provider.Conjur.Auth = esv1beta1.ConjurAuth{
...
// func (s conjurProvider) CreateKubernetesAuthStore(v *addon.Vault, ns string) {
